...
class ImagePredictor {
    ...
    static func createImageClassifier() -> VNCoreMLModel {
        ...
        let imageClassifierWrapper = try? FaceDetection3(configuration: defaultConfig)
    ...
    /// - Tag: makePredictions
    func makePredictions(for photo: CVPixelBuffer, completionHandler: @escaping ImagePredictionHandler) throws {
        let orientation = CGImagePropertyOrientation(rawValue: 1)!

        let imageClassificationRequest = createImageClassificationRequest()
        predictionHandlers[imageClassificationRequest] = completionHandler

        let handler = VNImageRequestHandler(cvPixelBuffer: photo, orientation: orientation)
